export const metadata = {
  title: "Services | Hair International & Beauty",
  description:
  ...
  return (
    <section className="py-12">
      <div className="container-responsive">
        ...
        <p className="mb-6">
          Looking for the <strong>best hair salon in Port Elizabeth</strong>? Hair International & Beauty offers a full
          range of hair and beauty services tailored to your needs. From precision haircuts to rich color and restorative
          treatments, our team focuses on healthy hair and beautiful results.
        </p>

        <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 list-none p-0">
          {services.map((s) => (
            <li key={s.name} className="border rounded-xl p-5 bg-white shadow-sm">
              <h2 className="text-xl font-semibold">{s.name}</h2>
              <p className="mt-2 text-gray-700">{s.description}</p>
              <p className="mt-2 text-xs text-gray-500">Keywords: {s.keywords.join(", ")}</p>
            </li>
          ))}
        </ul>
        ...
        <div className="mt-12">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <blockquote className="border-l-4 border-green-600 pl-4">
              “Best colour I’ve had in years. The team really listened and my hair feels so healthy.” — Local guest
            </blockquote>
            <blockquote className="border-l-4 border-green-600 pl-4">
              “Friendly, professional, and fast. I won’t go anywhere else in Port Elizabeth.” — Community member
            </blockquote>
          </div>
        </div>
      </div>
